...
:root{--bg:#0a0e17;--s1:#111827;--s2:#1a2332;--bd:#2a3a4e;--pr:#3b82f6;--ok:#10b981;--wn:#f59e0b;--er:#ef4444;--tx:#e5e7eb;--dm:#9ca3af}
...
.tg-link{display:flex;align-items:center;gap:5px;color:#2AABEE;font-size:12px;text-decoration:none;padding:4px 10px;background:rgba(42,171,238,.1);border-radius:20px;border:1px solid rgba(42,171,238,.3)}
...
.tg-save-btn:hover{border-color:#2AABEE;color:#2AABEE}
...
.footer a{color:#2AABEE;text-decoration:none;font-size:13px}
...
<h1>&#128269; IP Scanner</h1>
<div class="hd-r">
<a class="tg-link" href="https://t.me/Net4All_None" target="_blank">&#9992;&#65039; @Net4All_None</a>
<div class="st"><div class="dt" id="dot"></div><span id="stx">Ready</span></div>
</div>
</div>
<div class="wr">
<div>
<div class="cd"><div class="ct">Settings</div><div class="cc">
<label>Mode</label>
...
<div id="cp"><label>IPs (one per line)</label>
...
<div class="chk"><input type="checkbox" id="dohttp">
<label for="dohttp" style="margin:0">HTTP check (detect CDN)</label></div>
<div class="chk"><input type="checkbox" id="sendtg" onchange="TGT()">
<label for="sendtg" style="margin:0">&#128226; Telegram</label></div>
...
<div class="pgs"><span>Scanned: <b id="ps">0</b>/<b id="pt2">0</b></span>
<span>Found: <b id="pf">0</b></span></div>
</div>
</div></div>
<div class="cd"><div class="ct">Log</div>
<div class="cc" style="padding:5px"><div class="lg" id="lg"></div></div></div>
<div class="cd"><div class="ct">Saved Files <span style="font-size:10px;color:var(--pr);cursor:pointer" onclick="LF()">refresh</span></div>
<div class="cc"><div class="fl" id="fl">No files</div></div></div>
</div>
<div>
<div class="cd" style="min-height:400px">
<div class="ct"><span>Results (sorted by speed)</span>
...
<div class="tw">
<table><thead><tr>
<th onclick="SR(0)">#</th><th onclick="SR(1)">IP</th><th onclick="SR(2)">Port</th>
<th onclick="SR(3)">Latency</th><th onclick="SR(4)">Provider</th>
<th onclick="SR(5)">HTTP</th><th onclick="SR(6)">TLS</th><th onclick="SR(7)">Server</th>
</tr></thead>
<tbody id="tbd"><tr><td colspan="8" style="text-align:center;padding:30px;color:var(--dm)">Start scan to see results</td></tr></tbody>
</table></div>
<div class="sp" id="sv"><b>&#128190; Save Results</b>
<div class="so">
<div class="svb" onclick="SV('txt')">&#128196; TXT</div>
<div class="svb" onclick="SV('csv')">&#128202; CSV</div>
<div class="svb" onclick="SV('json')">&#128295; JSON</div>
</div>
<button class="tg-send-btn" id="tgSendBtn" onclick="sendTG()">&#9992;&#65039; Send to Telegram</button>
</div>
</div></div>
</div>
</div>
<div class="footer"><a href="https://t.me/Net4All_None" target="_blank">&#9992;&#65039; Telegram: @Net4All_None</a></div>
...
try{localStorage.setItem('tg_token',token);
localStorage.setItem('tg_channel',channel);
TT('Telegram settings saved!');L('Telegram settings saved','info')}
catch(e){TT('Cannot save (private mode?)')}}

function clearTG(){
try{localStorage.removeItem('tg_token');
localStorage.removeItem('tg_channel');
document.getElementById('tgtoken').value='';
document.getElementById('tgchannel').value='';
TT('Saved settings cleared!');L('Telegram settings cleared','info')}
catch(e){}}

function loadTG(){
try{var token=localStorage.getItem('tg_token');
...
var channel=document.getElementById('tgchannel').value.trim();
if(!token||!channel){
document.getElementById('sendtg').checked=true;TGT();
TT('Enter Bot Token and Channel ID first!');return}
sending=true;
document.getElementById('tgSendBtn').disabled=true;
document.getElementById('tgSendBtn').textContent='Sending...';
L('Sending to Telegram...','info');
sk.emit('send_telegram',{
tg_token:token,tg_channel:channel,
operator:document.getElementById('opsel').value})}

function loadOps(){
fetch('/api/operators').then(function(r){return r.json()}).then(function(ops){
var sel=document.getElementById('opsel');sel.innerHTML='';
for(var k in ops){var o=document.createElement('option');o.value=k;o.textContent=ops[k];sel.appendChild(o)}}).catch(function(){})}

function I(){
sk=io({transports:['websocket','polling'],reconnection:true});
sk.on('connect',function(){ok=true;L('Connected','info');SS('Ready',0);
fetch('/api/ranges').then(function(r){return r.json()}).then(function(d){R=d}).catch(function(){});
loadOps();LF();loadTG()});
sk.on('disconnect',function(){ok=false;L('Disconnected','er');SS('Offline',0)});
sk.on('scan_started',function(d){SS('Scanning...',1);
document.getElementById('goBtn').disabled=true;document.getElementById('stBtn').disabled=false;
document.getElementById('pg').classList.add('on');document.getElementById('pt2').textContent=d.total;
document.getElementById('tgSendBtn').className='tg-send-btn';
L('Started: '+d.total+' IPs','info')});
sk.on('scan_progress',function(d){var p=Math.round(d.percent);
document.getElementById('pb').style.width=p+'%';document.getElementById('pb').textContent=p+'%';
document.getElementById('ps').textContent=d.scanned;document.getElementById('pf').textContent=d.found});
sk.on('ip_found',function(r){A.push(r);A.sort(function(a,b){return a.latency_ms-b.latency_ms});RN();
L('+ '+r.ip+':'+r.port+' '+r.latency_ms+'ms '+r.provider,'ok')});
sk.on('scan_complete',function(d){SS('Done',0);
document.getElementById('goBtn').disabled=false;document.getElementById('stBtn').disabled=true;
if(d.results&&d.results.length>0){A=d.results;A.sort(function(a,b){return a.latency_ms-b.latency_ms})}
RN();document.getElementById('sv').classList.add('on');
document.getElementById('tgSendBtn').className='tg-send-btn on';
L('Done! '+d.total_found+'/'+d.total_scanned,'info');TT('Done: '+d.total_found+' IPs')});
sk.on('scan_stopped',function(){SS('Stopped',0);
document.getElementById('goBtn').disabled=false;document.getElementById('stBtn').disabled=true;
document.getElementById('stBtn').innerHTML='&#9209; Stop';
if(A.length>0){document.getElementById('sv').classList.add('on');
document.getElementById('tgSendBtn').className='tg-send-btn on'}
L('Stopped. Found: '+A.length,'er');TT('Stopped!')});
sk.on('scan_error',function(d){SS('Error',0);
document.getElementById('goBtn').disabled=false;document.getElementById('stBtn').disabled=true;
L('Error: '+d.message,'er');TT(d.message)});
sk.on('save_ok',function(d){saving=false;SB(true);
L('Saved: '+d.filename+' ('+d.count+')','info');TT('Saved: '+d.filename);LF()});
sk.on('save_error',function(d){saving=false;SB(true);L('Save error','er');TT('Error!')});
sk.on('telegram_ok',function(d){sending=false;
document.getElementById('tgSendBtn').disabled=false;
document.getElementById('tgSendBtn').innerHTML='&#9992;&#65039; Send to Telegram';
L('Telegram: sent! ('+d.count+' IPs)','info');TT('Sent to Telegram!')});
sk.on('telegram_error',function(d){sending=false;
document.getElementById('tgSendBtn').disabled=false;
document.getElementById('tgSendBtn').innerHTML='&#9992;&#65039; Send to Telegram';
L('Telegram error: '+d.message,'er');TT('Telegram error: '+d.message)})}

function SM(m){M=m;var t=document.getElementById('tabs').children;
...
var h='';for(var i=0;i<r.length;i++)
h+='<label class="ri"><input type="checkbox" checked class="rc" value="'+r[i]+'"> '+r[i]+'</label>';
document.getElementById('rl').innerHTML=h}
function TA(){var c=document.querySelectorAll('.rc');var a=true;
for(var i=0;i<c.length;i++)if(!c[i].checked){a=false;break}
for(var i=0;i<c.length;i++)c[i].checked=!a}
function GO(){
if(!ok){TT('Not connected!');return}
var sr=[];var ch=document.querySelectorAll('.rc:checked');
for(var i=0;i<ch.length;i++)sr.push(ch[i].value);
var data={mode:M,ips:document.getElementById('ipIn').value,
ports:document.getElementById('pt').value,threads:document.getElementById('th').value,
timeout:document.getElementById('to').value,sample:document.getElementById('sn').value,
do_http:document.getElementById('dohttp').checked,
operator:document.getElementById('opsel').value,selected_ranges:sr};
if(M==='custom'&&!data.ips.trim()){TT('Enter IPs!');return}
A=[];document.getElementById('tbd').innerHTML='';
document.getElementById('sv').classList.remove('on');
document.getElementById('tgSendBtn').className='tg-send-btn';
document.getElementById('pb').style.width='0';document.getElementById('pb').textContent='0%';
document.getElementById('ps').textContent='0';document.getElementById('pf').textContent='0';
sk.emit('start_scan',data);L('Starting...','info')}
function STP(){L('Stop requested...','er');
document.getElementById('stBtn').disabled=true;
document.getElementById('stBtn').textContent='Stopping...';
sk.emit('stop_scan')}
function SB(en){var b=document.querySelectorAll('.svb');
for(var i=0;i<b.length;i++){b[i].style.opacity=en?'1':'0.4';b[i].style.pointerEvents=en?'auto':'none'}}
function SV(fmt){if(saving){TT('Saving...');return}
if(A.length===0){TT('No results!');return}
saving=true;SB(false);L('Saving '+fmt+'...','info');
sk.emit('save_results',{format:fmt})}
function RN(){var f=document.getElementById('flt').value.toLowerCase();
var res=A;if(f){res=[];for(var i=0;i<A.length;i++){var r=A[i];
if(r.ip.indexOf(f)>=0||r.provider.toLowerCase().indexOf(f)>=0||(r.server_header||'').toLowerCase().indexOf(f)>=0)res.push(r)}}
var h='';var max=Math.min(res.length,500);
for(var i=0;i<max;i++){var r=res[i],rk=i+1;
...
function SR(c){var ks=['','ip','port','latency_ms','provider','http_status','tls_valid','server_header'];
var k=ks[c];if(!k)return;SD[k]=!SD[k];
A.sort(function(a,b){var x=a[k],y=b[k];
if(typeof x==='string')return SD[k]?x.localeCompare(y):y.localeCompare(x);
return SD[k]?x-y:y-x});RN()}
function LF(){fetch('/api/files').then(function(r){return r.json()}).then(function(fs){
var el=document.getElementById('fl');if(!fs.length){el.innerHTML='No files';return}
...
function L(m,t){var b=document.getElementById('lg');var d=document.createElement('div');
d.className='le '+(t||'info');d.textContent='['+new Date().toLocaleTimeString()+'] '+m;
b.appendChild(d);b.scrollTop=b.scrollHeight;while(b.children.length>100)b.removeChild(b.firstChild)}
function SS(t,a){document.getElementById('stx').textContent=t;
document.getElementById('dot').className=a?'dt on':'dt'}
function TT(m){var t=document.createElement('div');t.className='tt';t.textContent=m;
